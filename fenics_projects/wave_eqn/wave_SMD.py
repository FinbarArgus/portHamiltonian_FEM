...
# V6 output dir for loading
V6outputDir = 'output_' + 'V6' + '_' + case + 'DDNN'
...
pT_L_q_temp_left = 0.5*dt*c_squared*dot(q_temp, n)*forceExpression*ds(0)
pT_L_q_temp_gen = 0.5*dt*c_squared*q_bNormal*p_n*ds(1)
pT_L_q_temp_right = 0.5*dt*c_squared*dot(q_temp, n)*mirrorBoundary*ds(2)

# These are the energy contributions from the left, general and right boundaries for the second half timestep
pT_L_q_left = pT_L_q_temp_left + 0.5*dt*c_squared*dot(q_temp, n)*forceExpression*ds(0)
pT_L_q_gen = pT_L_q_temp_gen + 0.5*dt*c_squared*q_bNormal*p_*ds(1)
pT_L_q_right = pT_L_q_temp_right+ 0.5*dt*c_squared*dot(q_temp, n)*mirrorBoundary*ds(2)
...
if case=='rectangle' and boundaryType.startswith('DDNN'):
    ...
    H_array_V6 = np.load(os.path.join(V6outputDir, 'H_array.npy'))
    # Load Energy values
    ...
    E_array_V6 = np.load(os.path.join(V6outputDir, 'E_array.npy'))
...
if case == 'rectangle' and boundaryType.startswith('DDNN'):
    ...
    line_V6, = ax.plot([], lw=0.5, color='k',linestyle='--', label='2nd-order Stormer-Verlet Symplectic, Strong-Weak')

line, = ax.plot([], lw=0.5, color='k', linestyle='-', label='2nd-order Stormer-Verlet Symplectic, Weak')
...
if case == 'rectangle' and boundaryType.startswith('DDNN'):
    ...
    line_V6.set_data(H_array_V6[:,1], H_array_V6[:, 0])
...
plt.plot(t_vec, HGen_vec, lw=0.5, color='b', label='Neumann boundary (Top/Bottom)')
...
plt.plot(E_array_V6[:,1], E_array_V6[:,0], lw=0.5, color='k', linestyle='--', label='2nd-order Stormer-Verlet Symplectic, Strong-Weak')
plt.plot(t_vec, E_vec, lw=0.5, color='k', label='2nd-order Stormer-Verlet Symplectic, Weak')
